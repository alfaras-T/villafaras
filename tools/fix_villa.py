...
FIXES = {
    "185": {"name": "SAUNA VILLA 然",
            "reason": '**DB の住所は施設ではなく運営会社の管理事務所だった。住所の誤りの新しい型。**\n\n楽天の設備ページを全件走査したとき、ページ上の住所とDBの `addr` が食い違った（DB「長野市**北石堂町1191-20**」／楽天「長野市**県町466**」）。公式を見ると両方が載っていて、役割が違った。\n\n  「Access アクセス **住所：長野県長野市南長野県町４６６**」\n  「Company 運営会社 株式会社ライフ・パートナー … **管理事務所 長野市北石堂町１１９１－２０**」\n\n一休 00052003 も「〒380-0838 長野県長野市**南長野県町４６６**」で、**公式・一休・楽天の3つが一致する。**\n\n**座標が決定的だった。** 保存座標（36.65349, 138.183464）は Google Places のsearchText（施設名＋住所）で引いたもので、\n\n  「長野市南長野県町466」をGSIで引いた点 … **0.05 km**\n  「長野市北石堂町1191-20」をGSIで引いた点 … **7.41 km**（しかも解決できず「長野市北郷」に落ちる）\n\n**施設名で引いたので座標のほうが正しく、住所だけが事務所を指していた。**正しい大字は「**南長野**県町」で、「県町」だけでは市の重心に落ちる。チャネルAの導出値は座標から出しているので影響を受けていない。（2026-09確認）',
            "set_villa": {"addr": "長野県長野市南長野県町466"},
            },
    "198": {"name": "T&A Resort&Sauna KARUIZAWA",
            "reason": '**DB の住所に番地が無く（「長野県北佐久郡御代田町塩野」）、住所照合の対象から外れていたので今まで気づかなかった。**\n\n楽天の設備ページ（`HOTEL/199893`）が「〒389-0201 長野県北佐久郡御代田町**塩野482-90**」で、施設名も「Ｔ＆Ａ\u3000Ｒｅｓｏｒｔ＆Ｓａｕｎａ\u3000ＫＡＲＵＩＺＡＷＡ」と完全一致する。**この施設は `official` を持っておらず**、ota は airbnb / rakuten / expedia の3つだけで、airbnb は自動アクセスを拒否し expedia は429を返す。\n\n**座標で裏を取った。** 保存座標（36.340988, 138.501984）から\n\n  「御代田町塩野482-90」をGSIで引いた点 … **0.18 km**\n  「御代田町塩野」（大字のみ）をGSIで引いた点 … **3.04 km**\n\n**番地を足すと座標に寄る。** 大字だけの住所は地図が3km離れた場所を指していた。（2026-09確認）',
            "set_villa": {"addr": "長野県北佐久郡御代田町塩野482-90"},
            },
}
