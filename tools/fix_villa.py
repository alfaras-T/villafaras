...
FIXES = {
    "14": {"name": "Sea by TORAMII",
            "reason": "airbnb が広告クリック計測の中継URL（google.com/aclk）。候補は https://www.airbnb.com/rooms/1201744670802280048 。**5件とも現状は壊れたリンクで、利用者に見えたまま**だったので削除する。方針は「差し替えを試み、見つからなければ消す」。差し替え候補は見つかっているが、**Airbnb が自動ブラウザを拒否する（どのURLでも検索トップへリダイレクトし、Wayback にもキャッシュが無い）ため住所の実機照合ができず**、「独立した2回以上の検索で一致した値だけ採る」を満たせない。検索索引のタイトル一致だけで入れると、同名・同地名の別施設を指す危険がある（同じ調査で id=204 に「COCO VILLA MINAMIATAMI」という地名だけ同じ別ブランド施設が紛れ込んだ）。**候補URLは data/desk-research.js に残してあるので、実機の Chrome 拡張で住所を照合できたときに入れ直せる。**",
            "remove_ota": ["airbnb"],
            },
    "118": {"name": "SANU 2nd Home 那須1st",
            "reason": "airbnb キーに Booking.com のURLが入っていた（同じ施設の booking キーと同一値）。候補は https://www.airbnb.com/rooms/1375060797629253410 。**5件とも現状は壊れたリンクで、利用者に見えたまま**だったので削除する。方針は「差し替えを試み、見つからなければ消す」。差し替え候補は見つかっているが、**Airbnb が自動ブラウザを拒否する（どのURLでも検索トップへリダイレクトし、Wayback にもキャッシュが無い）ため住所の実機照合ができず**、「独立した2回以上の検索で一致した値だけ採る」を満たせない。検索索引のタイトル一致だけで入れると、同名・同地名の別施設を指す危険がある（同じ調査で id=204 に「COCO VILLA MINAMIATAMI」という地名だけ同じ別ブランド施設が紛れ込んだ）。**候補URLは data/desk-research.js に残してあるので、実機の Chrome 拡張で住所を照合できたときに入れ直せる。**",
            "remove_ota": ["airbnb"],
            },
    "126": {"name": "御宿 憩（OYADO IKOI）",
            "reason": "airbnb キーに Booking.com のURLが入っていた（同じ施設の booking キーと同一値）。候補は https://www.airbnb.com/rooms/1052358078081107159 。**5件とも現状は壊れたリンクで、利用者に見えたまま**だったので削除する。方針は「差し替えを試み、見つからなければ消す」。差し替え候補は見つかっているが、**Airbnb が自動ブラウザを拒否する（どのURLでも検索トップへリダイレクトし、Wayback にもキャッシュが無い）ため住所の実機照合ができず**、「独立した2回以上の検索で一致した値だけ採る」を満たせない。検索索引のタイトル一致だけで入れると、同名・同地名の別施設を指す危険がある（同じ調査で id=204 に「COCO VILLA MINAMIATAMI」という地名だけ同じ別ブランド施設が紛れ込んだ）。**候補URLは data/desk-research.js に残してあるので、実機の Chrome 拡張で住所を照合できたときに入れ直せる。**",
            "remove_ota": ["airbnb"],
            },
    "227": {"name": "マイグレA5",
            "reason": "airbnb が広告クリック計測の中継URL（google.com/aclk）。候補は https://www.airbnb.jp/rooms/1293928853355766178 。**5件とも現状は壊れたリンクで、利用者に見えたまま**だったので削除する。方針は「差し替えを試み、見つからなければ消す」。差し替え候補は見つかっているが、**Airbnb が自動ブラウザを拒否する（どのURLでも検索トップへリダイレクトし、Wayback にもキャッシュが無い）ため住所の実機照合ができず**、「独立した2回以上の検索で一致した値だけ採る」を満たせない。検索索引のタイトル一致だけで入れると、同名・同地名の別施設を指す危険がある（同じ調査で id=204 に「COCO VILLA MINAMIATAMI」という地名だけ同じ別ブランド施設が紛れ込んだ）。**候補URLは data/desk-research.js に残してあるので、実機の Chrome 拡張で住所を照合できたときに入れ直せる。**",
            "remove_ota": ["airbnb"],
            },
    "228": {"name": "マイグレパノラマ",
            "reason": "airbnb が airbnb.jp/s/homes の検索結果ページで施設に到達しない。候補は https://www.airbnb.jp/rooms/1406110484125312611 。**5件とも現状は壊れたリンクで、利用者に見えたまま**だったので削除する。方針は「差し替えを試み、見つからなければ消す」。差し替え候補は見つかっているが、**Airbnb が自動ブラウザを拒否する（どのURLでも検索トップへリダイレクトし、Wayback にもキャッシュが無い）ため住所の実機照合ができず**、「独立した2回以上の検索で一致した値だけ採る」を満たせない。検索索引のタイトル一致だけで入れると、同名・同地名の別施設を指す危険がある（同じ調査で id=204 に「COCO VILLA MINAMIATAMI」という地名だけ同じ別ブランド施設が紛れ込んだ）。**候補URLは data/desk-research.js に残してあるので、実機の Chrome 拡張で住所を照合できたときに入れ直せる。**",
            "remove_ota": ["airbnb"],
            },
}
