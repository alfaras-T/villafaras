...
FIXES = {
    "115": {"name": "有形文化財ホテル 飯塚邸",
            "reason": "**addr が誤っている。「栃木県那須郡那珂川町矢又1948」→「栃木県那珂川町馬頭360」。** 公式サイトを直接取得したところ住所は「栃木県那珂川町馬頭３６０」で、**「矢又」の語はページのどこにも登場しない**。一休の掲載住所も馬頭360。\n\n**チャネルAの導出値は無事だった。** 保存済みの座標（lat 36.737229 / lng 140.169408）を国土地理院のジオコーディングと突き合わせると、馬頭360（lat 36.737442 / lng 140.169449）と約25mで一致し、矢又1948（lat 36.711922 / lng 140.215530）とは約5km離れている。座標は施設名＋住所で引いているため、住所が誤っていても施設名が正しい場所に解決していた。したがって elevation / supermarket / conveni / ic / station / onsen は再計算不要で、表示用の住所文字列だけを直す（2026-09確認）",
            "set_villa": {"addr": "栃木県那珂川町馬頭360"}},
}
