...
FIXES = {
    "183": {"name": "Hakuba Amber Resort",
            "reason": "**sauna_exists を yes から no に訂正する。別の建物の記述を読んでいた。**\n\n公式 jadehotelgroup.com は未編集のプレースホルダーで機能しないが、運営の予約エンジンで住所「830-90 Hokujō, Hakuba」を引くと**3つの別物件**が登録されている。\n\n  property/67「Hakuba Amber Resort by Jade Villa」… 2LDK 最大6名**115㎡** / 3LDK 最大12名**160㎡** … **設備一覧にサウナの記載なし**\n  property/62「JADE Amber Resort Premium」… 最大13名 5寝室3浴室 **265㎡** … サウナあり\n  property/64「JADE Amber Resort Luxury」… 最大12名 5寝室5浴室 **330㎡** … 記載なし\n\n**DBの出典である一休 00051318 の部屋タイプは「最大6名 2LDKシャワールーム付シャレー」115平米＋「最大12名 3LDKでゆったり家族風呂付きシャレー」160平米で、面積が property/67 と完全一致する**（265㎡・330㎡の2物件とは一致しない）。つまり id=183 は property/67。\n\n**そしてその一休ページ自身が「宿の施設・設備」欄で「サウナ　なし」と明記している。**\n\nサウナがあるのは間取り・面積の合わない別物件（property/62）で、既存値はそれを読んだものとみられる。**同一住所に複数物件が並ぶブランドでの取り違え。** サウナタグも外す。（2026-09確認）",
            "set_spec": {"sauna_exists": {"v": "no", "src": "desk", "at": "2026-09",
                                          "url": "https://www.ikyu.com/00051318/"}},
            "remove_tags": ["sauna"],
            },
    "229": {"name": "WEAZER西伊豆",
            "reason": "**sauna_exists を room から no に訂正する。公式が明示的に否定している。**\n\n  公式chillnn のFAQ「サウナはありますか？」→「**WEAZER Villaのお部屋にはサウナはございません。**WEAZER 廻のお部屋には客室内にサウナ(定員2名・95℃)がございます。」\n\n**DBはこの施設を2エントリに分けている。** id=229 が「Villa」（75㎡・定員1〜3名）、id=230 が「廻」（142㎡・定員1〜4名）。id=230 の既存値（sauna_temp=95 / sauna_cap=2）がこの FAQ の回答と完全に一致しており、対応関係が裏付けられる。\n\n**2026-08 にこの値を no から room に訂正した経緯があるが、それは誤りだった。** 当時は一休の記載を根拠にしたが、一休は Villa と廻を1施設として扱っている。**エントリが分かれている以上、id=229 が指すのは Villa 単体**なので、公式の明示的な否定に従って no に戻す。サウナタグも外す。（2026-09確認）",
            "set_spec": {"sauna_exists": {"v": "no", "src": "desk", "at": "2026-09",
                                          "url": "https://www.chillnn.com/ja/1836d2246923a9"}},
            "remove_tags": ["sauna"],
            },
    "277": {"name": "No.12 Kashima Fan Zone",
            "reason": "**sauna_exists を shared から yes に訂正する。全客室に個室サウナが付いている。**\n\n  「ご宿泊のお客様の**お部屋にはそれぞれ個室サウナ**もございます」（/areamap/）\n  「**全部屋個室サウナは付いております**」（/stay/）\n  「**客室内サウナ**に設置された制御ユニットがデータを解析し…」（予約エンジンの施設詳細）\n\n共用の大型サウナ3種（騒・黙・動）とは**別に**、全8棟の客室すべてに専用サウナが付属する。判定基準の yes（客室・棟にサウナがある）に該当する。**共用施設が併存していても、客室にサウナがあれば yes。** サウナタグも足す。\n\nCLAUDE.md に「共用温浴施設と客室サウナが併存する（No.12 Kashima Fan Zone）」を**DB構造の限界の例**として挙げていたが、この項目に関しては限界ではなく**判定基準どおりに yes と決められる**。（2026-09確認）",
            "set_spec": {"sauna_exists": {"v": "yes", "src": "desk", "at": "2026-09",
                                          "url": "https://no-12.jp/stay/"}},
            "add_tags": ["sauna"],
            },
    "254": {"name": "伊豆グランヴィレッジ　グランピング",
            "reason": "pet_ok=yes を記録したのでペットタグを足す。「Dog tent style ドッグテントスタイル　愛犬と一緒にご宿泊いただけるドームタイプのお部屋です。専用ドッグラン付きで…」（2026-07-18新設）。**一部の客室タイプ限定でも pet_ok は yes** とする先例に従う。（2026-09）",
            "add_tags": ["pet"],
            },
}
