...
FIXES = {
    "204": {"name": "オーシャンビュー南熱海",
            "reason": "**出典URLが付いているのに、そのページの記載と違っていた。** 既存値12は公式ページ resolstay.jp/details/minamiatami/ を出典に持つが、そのページの表示は「定員 8名」。交差検証でDOMを直接調べたところ `<p class=\"label\">定員</p><p class=\"value\">8名</p>` が1組あるだけで、括弧書きの「推奨N名」は存在しない。ページ全文を「推奨」「最大」「上限」「12名」で検索しても、人数に関する記載は 8名 の1件だけだった（「推奨」の4件はすべて「お車でのお越しを推奨しております」等でアクセスの話）。寝具は「クイーン×2・布団×4」で8名と整合する。\n\n**当初は姉妹施設 id=205 のページ書式「定員 10名（推奨8名）」から、調査側が推奨人数を読んだ疑いを持ったが、交差検証でこれは棄却された。** resolstay.jp は括弧付き併記を使うページと使わないページが混在しており、id=204 は後者。既存値12の出所は不明。（2026-09確認）",
    ...
    "193": {"name": "SAUNA FOREST CABIN 軽井沢御代田",
            "reason": "**coldbath を bath から tub に訂正する。** 公式サイト全ページ（トップ／私たちについて／よくある質問／アクセス／施設案内／会議研修合宿利用）を横断検索して**「水風呂」は0件**。冷却設備は一貫して「ジャグジー」と呼ばれている。\n\n  よくある質問「Q部屋の設備には何がありますか？」→「Aサウナ、ジャグジー、全館空調システム、洗濯機、…」\n  サイト共通の meta description →「長野県 軽井沢 御代田のサウナとジャグジー コテージ 森の声が聞こえる場所」\n  一休 →「身体を芯から温めるサウナを楽しんだら、**ジャグジーでクールダウン**。」\n\n**名詞が何かで決めるという規約に照らすとジャグジー＝tub。** 「水風呂」の語がどこにも無いので、ジャグジーとは別に専用の水風呂があるという根拠も無い。棟差も無く、3棟とも「ジャグジー」の呼称で一貫している（FAQ の設備回答はベッド数などと違い棟別に分かれていない）。既存値 bath は出典URLの無い2026-07の一括投入分だった。\n\n**なお sauna_type=barrel は裏付けが取れなかった。** 公式・一休の両方を「バレル」「樽」「ドラム缶」「テント」「小屋」で検索して全て0件。値は残すが出典なしのまま。（2026-09確認）",
            "set_spec": {"coldbath": {"v": "tub", "src": "desk", "at": "2026-09",
                                      "url": "https://saunaforestcabin.com/qa/"}},
            },
}
