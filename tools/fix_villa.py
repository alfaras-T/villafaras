...
FIXES = {
    "13": {"name": "Ocean's Terrace TORAMII",
            "reason": "**`loyly: no` の根拠が消えた。削除する。** 波V14-15 でサウナイキタイの構造化テーブルが「セルフロウリュ＝無し」だったことを根拠に記録したが、**サウナイキタイの「無し」は未設定の既定表示であって否定の根拠にならない**ことが分かった。\n\n証明は id=70 The Pacific Retreat TATEYAMA。**イキタイ0・サ活0 で誰も情報を入れていない施設なのに、ロウリュ／オートロウリュ／セルフロウリュ／外気浴／休憩スペース／ウィスキング／イオンウォーターの7項目すべてが「無し」**と表示される。チェックボックスの既定がオフというだけで、「無い」と記録されたわけではない。\n\n**したがって `有り` だけが根拠になり、`無し` は不記載と同じ扱いにする。** 自分たちの「不記載は否定の根拠にならない」という規約そのもの。\n\nこの発見で、同じ波の他の判断も取り下げた。id=60 の `outdoor_rest: no` と `rest_chair: none`（記録前に却下）、id=16 の「outdoor_rest が既存 yes なのに構造化欄は無し」という食い違いの指摘、id=31 の「休憩スペースが明示的になし」という読み。**いずれも食い違いではなく未入力だった。**（2026-09確認）",
            "remove_spec": ["loyly"],
            },
    "62": {"name": "RICKA KATSUURA",
            "reason": "**`coldbath` を `bath` から `tub` に訂正した。設備の名詞が「五右衛門風呂」だった。** サウナイキタイの水風呂欄の備考に運用が書かれている。\n\n  「※チラー使用前に**五右衛門風呂**を水で満たしてください。故障の原因となりますので、空回しは避けてください。**五右衛門風呂を温浴で使用する場合は、チラーの運転をOFFにしてください。**」\n\n**チラーONで水風呂、OFFで温浴という兼用の浴槽**で、CLAUDE.md の「`tub` と `bath` は名詞が何かで決める。浴槽が水風呂を兼ねるなら `tub`」に当たる。id=236「アウトドアバス（水風呂・お湯張り可能）」→ `tub` と同じ型。\n\n同じ備考が既存の `chiller: yes` も裏付けている。（2026-09確認）",
            "set_spec": {"coldbath": {"v": "tub", "src": "desk", "at": "2026-09",
                                      "url": "https://sauna-ikitai.com/saunas/83377"}},
            },
}
