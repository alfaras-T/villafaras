...
FIXES = {
    "282": {"name": "GLAMPING KASHIMA 753 #00",
            "reason": "pet_ok=yes を記録したのでペットタグを足す。犬同伴専用の「ドッグコテージ（コテージI棟）」があり、公式に「**大切な愛犬とご一緒にお泊まりいただけるお部屋**」とある。**一部の客室タイプ限定のサービスでも pet_ok は yes** とする先例に従う（SANU 3件も「愛犬と泊まれる」プラン限定で yes にしている）。（2026-09）",
            "add_tags": ["pet"],
            },
}
