...
FIXES = {
    "19":  {"name": "GIFTHOUSE 館山 那古海岸",
            "reason": '**`validate.py` に入れた「`late_arrival` と `checkin` の整合」検査が出した4件の1つ。** 楽天トラベルの「最終チェックイン」と突き合わせて見つかった矛盾を、出典に当たって決着させた。（2026-09確認）\n\n**`checkin` が姉妹施設の値だった。** 公式 nagokaigan.gifthouse.jp は\n\n  「**チェックイン：15:00〜18:00** | チェックアウト：11:00」\n\nと明記している。DB の「15:00〜24:00」は**姉妹施設 id=20 GIFTHOUSE 2nd 館山 洲宮の値と完全に同じ**で、2026-08 の一次記録にも\n\n  「那古海岸は『3口IH』『室内サウナ』『**チェックイン15:00〜18:00**』に対し、洲宮は『ガスコンロ×1』『屋外薪バレルサウナ』『**15:00〜24:00**』」\n\nと書いてあった。**気づいていたのに `checkin` に反映していなかった。**\n\n`late_arrival: no` は正しいので触らない。**楽天は「最終：24:00」で公式と食い違う（楽天の最終チェックインは予約の受付上限であって施設の方針ではない）。**\n\nこの施設は CLAUDE.md が「サイト内で数値が食い違う」例として挙げてきたが、**実際には姉妹施設の値の混入だった。**',
            "set_villa": {"checkin": "15:00〜18:00"},
            },
    "43":  {"name": "Montevan RESORT VILLA",
            "reason": '**`validate.py` に入れた「`late_arrival` と `checkin` の整合」検査が出した4件の1つ。** 楽天トラベルの「最終チェックイン」と突き合わせて見つかった矛盾を、出典に当たって決着させた。（2026-09確認）\n\n**公式と楽天が一致し、DB の `checkin` だけが外れていた。** 公式 montevan.com は\n\n  「**チェックイン15:00〜20:00**、チェックアウト10:00（追加料金にてアーリーチェックイン/レイトチェックアウトも可能）」\n  「※**到着が17時半以降になる場合はサービスデスクまでご連絡ください**」\n\n楽天も「最終：20:00」で一致する。DB の「15:00〜24:00」を 15:00〜20:00 に直した。\n\n`late_arrival: no` は正しい（21時以降は受付時間外）。なお17時半以降の連絡要請は**`late_arrival` の想定する21時とは別の時刻**なので `contact` にはしない。\n\nこの施設はチェックイン処理を**別住所のサービスデスク**で行う（公式に住所が2つ併記されており、DBの「香191-3」がヴィラ本体、一休の「大賀170-2」がサービスデスク）。',
            "set_villa": {"checkin": "15:00〜20:00"},
            },
    "269": {"name": "THE LOOKOUT KUSATSU",
            "reason": '**`validate.py` に入れた「`late_arrival` と `checkin` の整合」検査が出した4件の1つ。** 楽天トラベルの「最終チェックイン」と突き合わせて見つかった矛盾を、出典に当たって決着させた。（2026-09確認）\n\n**`ok` は出典の読み違いだった。同じページが「20時30分まで」と明記している。**\n\n出典 aco.co.jp/id/67359.html には\n\n  「《チェックイン》 0.**鍵の受け取りは20時30分までに**お願いいたします。それ以降は閉店するので鍵が受け取れなくなってしまいます」\n  「夜の到着OK **20時30分までであればチェックイン可能です**」\n\nとある。**「夜の到着OK」という見出しだけを読んで `ok` にしたとみられる。**\n21時以降は鍵が受け取れないので `no` が正しい。DB の `checkin` 15:00〜20:00 とも整合する。\n\n公式 kitakaru.localinfo.jp は2施設のランディングページで本文832字しかなく、チェックインの記述が無い。**aco.co.jp が唯一の出典のままである。**',
            "set_spec": {"late_arrival": {"v": "no", "src": "desk", "at": "2026-09",
                                          "url": "https://www.aco.co.jp/id/67359.html"}},
            },
    "280": {"name": "一棟貸切宿　藤右衛門",
            "reason": '**`validate.py` に入れた「`late_arrival` と `checkin` の整合」検査が出した4件の1つ。** 楽天トラベルの「最終チェックイン」と突き合わせて見つかった矛盾を、出典に当たって決着させた。（2026-09確認）\n\n**根拠にした記述が公式から消えていた。値を削除して未調査に戻す。**\n\n2026-08 の一次記録は公式FAQ の「**チェックインは、15:00〜18:00です**」を根拠に`no` としていた。**現在の公式にその FAQ は存在しない。**「よくある質問」のリンクは `#faq` を指すが、そのアンカーに中身が無い。\n\n現行の「ご利用案内」はこうなっている。\n\n  「check チェックイン・アウト（**セルフ**） • **チェックイン：15:00〜** • チェックアウト：〜10:00」\n  「※**スマートキーのコード**は宿泊日当日にメールでお知らせします」\n\n**終了時刻が書かれていない。** スマートキーの無人チェックインに変わったとみられ、18:00 の締切はもう無い可能性が高い（楽天も「最終：24:00」、DB の `checkin` も15:00〜24:00）。**ただし「21時以降に着いてよい」と書いた文は無いので `ok` にもできない。**\n\n不記載は肯定の根拠にも否定の根拠にもならないので、**削除して未調査に戻す。**\n`checkin` 15:00〜24:00 は公式と矛盾しない（公式は開始時刻しか書いていない）ので据え置く。\n\n同じ改訂で「最大定員：20名」も確認できた（2026-09 に 5 → 20 に訂正済みの値と一致）。',
            "remove_spec": ["late_arrival"],
            },
}
