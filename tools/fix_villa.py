...
FIXES = {
    "109": {"name": "Kakoi 雪嶺",
            "reason": "**住所から大字「富士ケ嶺」が欠落していた。** 公式予約サイト（chillnn）の住所表記は「山梨県南都留郡富士河口湖町**富士ケ嶺**273-10」。過去に見つけた「町村名の欠落」型と同じ。\n\n国土地理院のジオコーディングで裏を取った。\n  保存座標                              (35.404150, 138.626206)\n  現 addr「富士河口湖町273-10」        (35.497223, 138.755005) … **15.60km**\n  新 addr「富士河口湖町富士ケ嶺273-10」 (35.414104, 138.626892) … **1.11km**\n\n**大字が無いと町の中心に落ちて15km以上ずれる。** 座標のほうは正しい（Places は施設名＋住所で引くので施設名で救われている）。チャネルAの導出値も座標由来なので影響を受けていない。（2026-09確認）",
            "set_villa": {"addr": "山梨県南都留郡富士河口湖町富士ケ嶺273-10"},
            },
    "199": {"name": "北軽井沢 貸別荘 FARMSIDE",
            "reason": "公式のアクセスページの表記「北軽井沢 貸別荘 FARM SIDE　群馬県吾妻郡嬬恋村**鎌原1054-116**」に合わせて番地を補う。国土地理院では -116 まで解決できず現 addr と同じ点を返すため座標との距離は変わらないが、公式の表記に揃えておく。\n\n**あわせて公式URLの `newsite.` サブドメインを調べたが、ステージング環境ではなかった。** `kitakaruizawafarmstay.com`（無印）は「北軽井沢 人間牧場【公式】」のブランドサイトで、**その無印ドメイン自身が部屋詳細・利用規約・問い合わせのリンク先として `newsite.` 側を使っている。** 古民家用の `yamaban.` サブドメインも同じ構成。意図的なサブドメイン分割で、現在の公式URLのままでよい。（2026-09確認）",
            "set_villa": {"addr": "群馬県吾妻郡嬬恋村鎌原1054-116"},
            },
    "126": {"name": "御宿 憩（OYADO IKOI）",
            "reason": "公式URLを英語版から日本語版に差し替える。DBは `stay-japan.tokyo/en/ikoi/` を指していたが、**日本語版 `stay-japan.tokyo/hotel/ikoi/` のほうが記載が大幅に詳しい**（設備一覧・予約制設備・入浴時間などが載る）。なお `/ikoi/`（hotel 無し）は 404。（2026-09確認）",
            "set_villa": {"official": "https://stay-japan.tokyo/hotel/ikoi/"},
            },
}
