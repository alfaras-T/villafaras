...
FIXES = {
    "69": {"name": "primera villa",
            "reason": "**一度『サウナは実在しない』と結論して消した値を、根拠を付けて戻した。** 当時の根拠は**公式サイトの不記載だけ**で、『不記載は否定の根拠にならない』という自分たちの規約に反していた。一休 00052313 に複数箇所の肯定がある。\n\n  FAQ「サウナはありますか？」→「**ございます。**」\n  客室紹介「ココロまで整うサウナ時間　**セルフロウリュウが楽しめるプライベートサウナ**。」\n  施設・サービス欄「サウナ / 屋外プール」\n\n公式 primera-gr.co.jp は**グループサイトではなく単独施設のサイト**で、トップから定員12名・4LDK・FAQ がそのまま出る。富津市金谷はサウナ付きヴィラが密集し検索結果に別施設が混ざりやすいが、一休の住所「千葉県富津市金谷530-1」がDBと完全一致することを確認した。\n\n**1棟貸しで『プライベートサウナ』と明記されているので `shared` ではなく `yes`。** `sauna_exists=yes` に伴い `sauna` タグを付ける。（2026-09確認）",
            "add_tags": ["sauna"],
            },
    "285": {"name": "上小川レジャーペンション",
            "reason": "**`sauna_exists` を `room` から `shared` に訂正した。** 旧値の出典は cottagelife.jp という第三者のまとめサイトだった。一次情報を当たると、サウナは敷地内の独立した貸切小屋で、共用のサインが揃っている。\n\n  「**1日2組限定**、ヒノキの香りとロウリュを楽しめる薪ストーブを使った本格貸切サウナ」\n  「午前の部〔10:00~12:30〕**午後の部**〔13:30~16:00〕」（2.5時間制）\n  「4名まで **10,000円**。1名追加／2,000円」\n  「『**なっぷ**』または『**LINE**』にてご予約ください」\n  サウナイキタイ「アウトドアサウナ：サウナ小屋（屋外・**水着着用**）」\n\n**予約制・時間制・別料金**は共用のサイン（CLAUDE.md）。加えて**1日2組限定＝1日に別の2組が使う**ので占有ではない。この施設はコテージ5種＋テントサイト＋グランピングで1組貸切でもない。\n\n**`room` の根拠とみられるメープルコテージは、見出しが「半露天風呂・サウナ付きの贅沢コテージ」なのに、同じページの備品一覧が「エアコン・ＡＣ電源・半露天風呂（ボディーソープ・シャンプー付）・トイレ・炭専用ＢＢＱコンロ・屋外テーブル・屋外チェア・寝具」でサウナを含まない。** 他の4コテージ（オリーブ・バンブー・プラム・ヴィレッジ）はいずれも見出しにも一覧にもサウナが無い。**公式サイト内で見出しと詳細が食い違っている型。**\n\n`shared` なので `sauna` タグを外す。（2026-09確認）",
            "set_spec": {"sauna_exists": {"v": "shared", "src": "desk", "at": "2026-09",
                                          "url": "https://www.kamiogawa.net/facility/%E3%82%B5%E3%82%A6%E3%83%8A/"}},
            "remove_tags": ["sauna"],
            },
    "274": {"name": "サンライズヴィラ大洗",
            "reason": "**`stove: electric` の出典が値を裏付けていなかった（『出典があるのに中身が無い』の4例目）。** 出典に持っていた `sunrise-villa.jp/facility/` の記載は\n\n  「世界的サウナヒーターブランド『**ＨＡＲＶＩＡ**』採用」\n\nだけで、**HARVIA は薪式と電気式の両方を製造しているため熱源は確定できない**（id=215 / 225 / 56 / 283 で採用を見送っている書き方そのもの）。\n\n値そのものは誤りではなく、サウナイキタイの構造化データが「対流式（ストーン）／**電気**／TV無」と持っている。公式の記載とも矛盾しない。**値は据え置き、出典をサウナイキタイに付け替えた。**\n\n過去3例（id=252 伊豆高原テントリゾート、id=204 オーシャンビュー南熱海、id=131 LEVATA）はいずれも値が誤りだったが、**今回は値が正しく出典だけが不適切**という初めての型。（2026-09確認）",
            "set_spec": {"stove": {"v": "electric", "src": "desk", "at": "2026-09",
                                   "url": "https://sauna-ikitai.com/saunas/79257"}},
            },
}
