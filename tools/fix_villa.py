...
FIXES = {
    "102": {"name": "SAUNEA白州",
            "reason": "**`rest_chair` を `chair` から `infinity` に訂正した。** 旧値は `at: '2026-07'` で**出典URLが無く**、誤りが集中する初期一括投入のコホートだった。\n\nサウナイキタイの休憩スペース備考は\n\n  「●外気浴　**寝転べるイス（フルフラット可）：6席**」\n\nの**1種類だけ**で、2種併記による判断保留にも当たらない。規約どおり `infinity`。\n\n同じページから `sauna_temp=96` / `water_temp=t1518`（15度）/ `water_depth=d6080` / `stove=electric` も記録した。（2026-09確認）",
            "set_spec": {"rest_chair": {"v": "infinity", "src": "desk", "at": "2026-09",
                                        "url": "https://sauna-ikitai.com/saunas/81542"}},
            },
    "99": {"name": "ハンズアウトドアリゾート",
            "reason": "**`sauna_type` を `tent` から `barrel` に訂正した。CLAUDE.md が既に「バレルで決着」と結論していたのに、値が古いまま残っていた。**\n\n旧値の出典は `hanz-odr.com/hanz-faqs/`。**CLAUDE.md はこの FAQ を「古い側」と判定している**（「テント側 … FAQ に更新日の表示なし。サウナイキタイの投稿2件も2023年でバレル開設と同時期」）。決め手は日付のある材料の偏りだった。\n\n  バレル側 … 2023-04-24 の開設告知、**2026-09-03 の値上げ告知**（現役稼働の証拠）、\n    本日の Activities Tour ページに**単独掲載**\n\n**サウナイキタイ側から独立の裏付けが出た。** 構造化データが2つのサウナ室に分かれており、\n\n  サウナ室1「開業から**3周年記念の際に設置された**バレルサウナ」\n  サウナ室2「テントサウナ」\n\n**バレルのほうが後発**と明記されている。サ活投稿はいずれも2023年でテント時代のもの。**古い記述が消されないまま残るという、この情報源の既知の癖どおり。**\n\n2室が併記されていることをもって「両方ある＝単一値にできない」とも読めるが、**公式の現行 Activities ページがバレル単独掲載**である以上、テントは提供終了とみるのが自然。（2026-09確認）",
            "set_spec": {"sauna_type": {"v": "barrel", "src": "desk", "at": "2026-09",
                                        "url": "https://sauna-ikitai.com/saunas/78811"}},
            },
}
