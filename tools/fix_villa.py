...
FIXES = {
    "191": {"name": "軽井沢 HOUSE VILLA",
            "reason": "**capacity=9 はどの棟の数字とも一致しない。** 公式の客室ページは6棟すべての定員を個別に出しており「Villa A 定員12名」「Villa B (サウナ付) 定員11名」「平屋棟 定員11名」「サウナ棟 サウナと水風呂付き 定員13名」「足湯棟 定員13名」「檜風呂棟 定員13名」。9 はどれにも当たらず、OTAの定員欄（1名〜9名）の遺物とみられる。「全棟利用で最大N名」の記載は無いので、棟別の数字しか無い場合は施設全体の最大を採るという先例に従い 13 とする。\n\n**あわせて stove=wood を削除する。居室の薪ストーブとの取り違えである。** 公式トップのコンセプト欄は「全棟に薪ストーブ」と書き、客室ページでも**サウナが無い Villa A・足湯棟・檜風呂棟を含む6棟すべての設備一覧に「薪ストーブ」が並んでいる。** サウナ自体の熱源を明記した記述はページ内に見つからない。この値は出典URLも無い2026-07の一括投入分だった。（2026-09確認）",
            "set_villa": {"capacity": "13"},
            "set_spec": {"capacity": {"v": 13, "src": "desk", "at": "2026-09",
                                      "url": "https://karuizawa-house-villa.com/room"}},
            "remove_spec": ["stove"],
            },
}
