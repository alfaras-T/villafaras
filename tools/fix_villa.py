...
FIXES = {
    "209": {"name": "伊豆高原プライム",
            "reason": "sauna_exists=yes は誤り。公式（リソルステイ）の施設ページには「サウナ」「水風呂」が一度も出てこない。同サイトは施設ごとに特徴を列挙するアイコン行を持ち、姉妹施設の熱海リゾートでは「BBQ／温泉／サウナ／絶景／テラス」とサウナが並ぶのに対し、伊豆高原プライムは「ペット／BBQ／温泉／デリバリー可／テラス」でサウナが無い。同一テンプレートの同一列挙でサウナが選択肢として実在するため範囲が一致し、否定値として記録できる。家具・家電・食器・調理器具まで網羅した設備一覧にも記載なし。chillnn予約ページ・温泉サウナ特集ページ・サウナイキタイでも確認できなかった。no に訂正しサウナタグを外す（2026-08確認）",
            "remove_tags": ["sauna"],
            "set_spec": {"sauna_exists": {"v": "no", "src": "desk", "at": "2026-08",
                                          "url": "https://www.resolstay.jp/details/v_izukougen/"}}},

    "201": {"name": "キュレーション熱海桃乃八庵",
            "reason": "sauna_exists=yes の裏付けが取れない。公式（curationhotels.com）の施設ページにサウナの記載が一度も無く、リソルステイ側の個別ページは存在せずトップへリダイレクトされる。ただしこのページは意匠中心で設備の網羅的な列挙を持たないため、否定値の条件（列挙形式かつ範囲が一致）を満たさない。判定基準どおり未調査に戻し、サウナタグも外す。あわせて capacity=8 も誤り。公式に「※桃乃八庵は海外拠点を持つオーナーの長期不在時にのみ、1棟貸しで宿泊いただける施設です。（定員4名、1泊20万円、長期割引あり）」「寝室 3部屋（定員最大6名）」とあり、8はどちらとも一致しない。最大と明記された6を採る（2026-08確認）",
            "remove_tags": ["sauna"],
            "remove_spec": ["sauna_exists"],
            "set_villa": {"capacity": "6"},
            "set_spec": {"capacity": {"v": 6, "src": "desk", "at": "2026-08",
